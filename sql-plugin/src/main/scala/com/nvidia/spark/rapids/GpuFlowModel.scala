...
/** Pure projected-gradient inner solver for one fixed Spark/AQE topology. */
private[rapids] object GpuFlowProjectedOptimizer {
  private val MaxIterations = 48
  private val MaxBacktrackingSteps = 24
  private val MinimumStep = 1e-9

  def optimize(
      work: GpuFlowStageWork,
      initial: GpuFlowControl,
      bounds: GpuFlowControlBounds): GpuFlowControl = {
    var selected = bounds.project(initial)
    var selectedEvaluation = GpuFlowStageModel.evaluate(work, selected)
    var iteration = 0
    var converged = false
    while (iteration < MaxIterations && !converged) {
      val movable = GpuFlowControl.Indices.filter { index =>
        selected.value(index) > 0.0 &&
          bounds.maximum.value(index) > bounds.minimum.value(index)
      }
      val maxGradient = movable.map(index => math.abs(selectedEvaluation.gradient.value(index)))
        .foldLeft(0.0)(math.max)
      if (maxGradient <= math.max(1.0, selectedEvaluation.predictedNanos) * 1e-12) {
        converged = true
      } else {
        var step = 1.0
        var accepted = false
        var backtracking = 0
        while (!accepted && backtracking < MaxBacktrackingSteps && step >= MinimumStep) {
          val trial = movable.foldLeft(selected) { case (control, index) =>
            val normalizedDirection = -selectedEvaluation.gradient.value(index) / maxGradient
            val current = selected.value(index)
            control.updated(index, current * math.exp(step * normalizedDirection))
          }
          val projected = bounds.project(trial)
          val evaluation = GpuFlowStageModel.evaluate(work, projected)
          if (evaluation.predictedNanos < selectedEvaluation.predictedNanos) {
            selected = projected
    ...
    quantizeIntegerControls(work, selected, bounds)
  }

  /** Enumerate floor/ceiling choices for integer-valued controls and retain the modeled optimum. */
  private def quantizeIntegerControls(
      work: GpuFlowStageWork,
      continuous: GpuFlowControl,
      bounds: GpuFlowControlBounds): GpuFlowControl = {
    val integerIndices = Seq(
      GpuFlowControl.ScanWindow,
      GpuFlowControl.GpuTasks,
      GpuFlowControl.ShuffleWindow).filter(index => continuous.value(index) > 0.0)
    val candidates = integerIndices.foldLeft(Seq(continuous)) { case (controls, index) =>
      controls.flatMap { control =>
        val value = control.value(index)
        Seq(math.floor(value), math.ceil(value)).distinct.map { rounded =>
          bounds.project(control.updated(index, rounded))
        }
      }
    }
    candidates.distinct.minBy(control => GpuFlowStageModel.evaluate(work, control).predictedNanos)
  }
}
