  ...
  override def newInstance(): SerializerInstance = {
    if (useKudo) {
      if (kudoMode == ShuffleKudoMode.GPU) {
        new KudoGpuSerializerInstance(metrics, dataTypes, kudoMeasureBufferCopy)
      } else {
...
private class KudoGpuSerializerInstance(
  ...
  override def serializeStream(out: OutputStream): SerializationStream = new SerializationStream {
    // Batches that were not serialized on the device (e.g. the adaptive compression
    // gate routed them through the configured CPU slicing path) are written by the
    // CPU kudo serializer onto the same stream. Kudo records are self-contained, the
    // CPU path writes straight to `out` with no buffering of its own, and the read
    // side dispatches per record, so the two record producers can interleave freely.
    private lazy val cpuFallback =
      new KudoSerializerInstance(metrics, dataTypes, measureBufferCopyTime)
        .serializeStream(out)
...
object KudoCompressedFrame {
  ...
  /** Magic number "KUDL" for lz4-compressed payloads. */
  val MAGIC_LZ4: Int = 0x4B55444C

  def magicFor(codec: String): Int = codec match {
    case "zstd" => MAGIC
    case "lz4" => MAGIC_LZ4
    case other => throw new IllegalArgumentException(s"unknown kudo compression codec: $other")
  }

  def codecFor(magic: Int): Option[String] = magic match {
    case MAGIC => Some("zstd")
    case MAGIC_LZ4 => Some("lz4")
    case _ => None
  }
...
case class KudoCompressedMeta(numRows: Int, uncompressedLength: Long, chunkSize: Int,
    codec: String = "zstd") {
  require(numRows >= 0, s"Invalid KUDZ row count: $numRows")
  require(uncompressedLength > 0,
    s"Invalid KUDZ uncompressed length: $uncompressedLength")
  require(chunkSize > 0, s"Invalid KUDZ chunk size: $chunkSize")
}
    ...
    if (streamClosed) {
      ...
      if (nextHeader.isEmpty && nextCompressed.isEmpty) {
        NvtxRegistry.READ_HEADER {
          ...
          magic.map(m => (m, KudoCompressedFrame.codecFor(m))) match {
            case None =>
              closeStream()
            case Some((_, Some(codec))) =>
              val numRows = dIn.readInt()
              val chunkSize = dIn.readInt()
              val uncompressedLength = dIn.readLong()
              val compressedLength = dIn.readInt()
              if (compressedLength <= 0) {
                throw new IOException(s"Invalid compressed kudo length: $compressedLength")
              }
              nextCompressed = Some((
                KudoCompressedMeta(numRows, uncompressedLength, chunkSize, codec),
                compressedLength))
            case Some(_) =>
              dIn.reset()
